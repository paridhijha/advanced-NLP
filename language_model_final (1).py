...
vocab_size=30
log_base=2
# Assumption : the following 3 training files are stored in a local directory, the same from where the program is run
...
# Other Remarks     : The function uses the library re and its function sub to perform the string operations. The regex expression is a global variable
...
# Function Purpose  : to pre-process the text line by line and save it in a new file using the output of the function preprocess_line()
...
# Other Remarks     : requires a valid file with at least one bigram, otherwise throws an error
...
# Other Remarks     : requires a valid file with at least one unigram, otherwise throws an error
...
#                     to store an inner dictionary. It then uses (C-2,C-1) as the key and
...
# Other Remarks     : it calls the function get_prob_of_trigram() internally to  calculate fetch the probability of a particular trigram.
...
# Other Remarks     : this is the solution for subpart of Q 3.3.3 which asks to calculate 2 character history of th. this function is called from the main
...
# Function Purpose  : to generate random output(300 characters) for each of the 3 languages using the 3 LMs trained before
# Function Input    : none
...
# Other Remarks     : it also prints the perplexity of all 3 models for the given test file. this function is the solution for subpart of Q 3.3.3
...
# Function Input    : the text for one among the 3 languages. the file is expected to be in a local directory
# Function Output   : a results file - perplexity_result_test_file.txt which is created in a local directory
# Other Remarks     : this is a variation of the function detect_lang_test_doc(). instead of a test file, it inputs a text string.
...
#                     new files which represent the lang models and stores them in local directory using print_estimated_log_prob() function
