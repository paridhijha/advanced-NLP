...
# references : https://web.stanford.edu/class/cs124/lec/languagemodeling.pdf
#              lang modelling lecture https://www.youtube.com/watch?v=s3kKlUBa3b0&list=PL6397E4B26D00A269&index=12
...
def detect_lang_test_doc(test_doc):
    ...
    perplex_en = calc_perplex_log_testfile(text,est_log_prob_dict_en)
    perplex_es = calc_perplex_log_testfile(text,est_log_prob_dict_es)
    perplex_de = calc_perplex_log_testfile(text,est_log_prob_dict_de)
...
def detect_lang_for_text(text):
    ...
    perplex_en = calc_perplex_log(text,est_log_prob_dict_en)
    perplex_es = calc_perplex_log(text,est_log_prob_dict_es)
    perplex_de = calc_perplex_log(text,est_log_prob_dict_de)
...
# Function Name     : calc_perplex()
# Function Purpose  :
# Function Input    :
# Function Output   :
# Other Remarks     :

def calc_perplex(text,prob_dict):

    if not prob_dict:
        print ("\n Error : The probability dictionary is empty. Program aborted. \n")
        sys.exit()
    # Comment : Check to see whether the text passed to this function is empty. If yes, print and error message and abort the function using sys.exit()
    if not text:
        print ("\n Error : Empty string. Cannot calculate perplexity. Program aborted. \n")
        sys.exit()
    # Comment : Check to see whether the text passed to this function is less that 2 characters since it will not be a trigram. If yes, print and error
    #           message and abort the function using sys.exit()
    if(len(text) < 3):
        print ("\n Error : String does not contain a valid trigram. Cannot calculate perplexity. Program aborted. \n ")
        sys.exit()
    # Comment : The text is represented as a string of characters ch1,ch2....chn
    ch1=text[0]
    ch2=text[1]
    num_trigrams = len(text)-2.0
    #print "Debug : Number of trigrams is ",num_trigrams
    product=1.0
    #print "Debug : Calculating perplexity for text = ",text,"of size=", num_trigrams
    for index in range(2,len(text)):
        ch1= text[index-2]
        ch2= text[index-1]
        ch3= text[index]
        trigram = ch1+ch2+ch3
        current_prob=get_prob_of_trigram(trigram,prob_dict)
        #print "Debug : Value returned from get_prob_of_trigram() is =",current_prob
        product *=current_prob
        #print "Debug : Current trigram=",trigram,"prob=",current_prob,"product=",product
    exponent=(1/num_trigrams)
    base=product
    #print "base is",base, " and exponent is ",exponent
    reciprocal=math.pow(base,exponent)
    if not reciprocal:
        print ("\n Error : The denominator cannot be zero as it will result in division by zero error. Cannot calculate perplexity. Program aborted. \n")
        sys.exit()
    perplexity =1/reciprocal
    # Comment : Perplexity is calculated using the formula PP(W) = (P(wi|wi-2,wi-1)
    return perplexity
...
def calc_perplex_log(text,log_prob_dict):
...
def calc_perplex_log_testfile(text,log_prob_dict):
...
def estimate_probabilities(raw_bigram_count, raw_trigram_count):
    ...
    for outer_key in vocab:
        ...
        for inner_key1 in vocab:
            for inner_key2 in vocab:
                ...
                if bigram_count > 0 :
                    #print "Debug : For trigram =",trigram," trigram count is ", trigram_count, "and bigram count is ",bigram_count
                    est_prob = trigram_count/bigram_count
                else:
                    est_prob = 0
                #print "Debug : For trigram =",trigram,"the estimated prob is",est_prob
                inner_dict[bigram] = est_prob
...
def estimate_probabilities_with_add1(raw_bigram_count, raw_trigram_count):
    if not raw_bigram_count:
        print ("\n Error : The raw_bigram_count dictionary is empty. Program aborted. \n")
        sys.exit()
    if not raw_trigram_count:
    ...
    for outer_key in vocab:
        ...
        for inner_key1 in vocab:
            for inner_key2 in vocab:
                bigram=inner_key1+inner_key2
                trigram=inner_key1+inner_key2+outer_key
                trigram_count = raw_trigram_count[trigram]+1
                bigram_count  = raw_bigram_count[bigram]+vocab_size
                #print "Debug : For trigram =",trigram," trigram count is ", trigram_count, "and bigram count is ",bigram_count
                est_prob_value = trigram_count/bigram_count
                if not est_prob_value:
                    print ("\n Error : The estimated probability cannot be zero. Program aborted. \n")
                    sys.exit()
                #print "Debug : For trigram =",trigram,"the estimated prob is",est_prob
                inner_dict[bigram] = est_prob_value
        prob_dict[outer_key]=inner_dict
    return prob_dict
...
def get_sum_prob(dict):
    if not dict:
        print ("\n Error : The probability dictionary is empty. Program aborted. \n")
        sys.exit()
    sum=0.0
    for outer_key,inner_dict in dict.iteritems():
        for inner_key,prob_value in inner_dict.iteritems():
            sum+=prob_value
    return sum
...
# Function Name     : print_estimated_prob()
# Function Purpose  : to print a given dictionary (both a standard and a nested one) in a file in the same local directory
# Function Input    : dictionary (var = dict), string value (var = lang) for English, Spanish or German
# Function Output   : none
# Other Remarks     : none

def print_estimated_prob(dict, lang):
    if not dict:
            print ("\n Error : The probability dictionary is empty. Program aborted. \n")
            sys.exit()
    filename = "model_prob_"+lang+".txt"
    outputfile= open(filename,'w')
    outputfile.write('\n')

    output.write("   Trigram       Probability")
    for outer_key,inner_dict in dict.iteritems():
        for inner_key,prob_value in inner_dict.iteritems():
...
def sort_estimated_log_prob(dict, lang):
    ...
    for outer_key,inner_dict in dict.iteritems():
        for inner_key,log_prob_value in inner_dict.iteritems():
            ...
            #print trigram,prob_value
            prob_dict[trigram]=prob_value
    
    by_prob_value=lambda i: float(prob_dict[i])
...
# Comment : Following is a test case for this program. The vocabulary is only 3 characters [ab,] and the input file is generated assuming the preprocessing is already done. The purpose of this test case is to check the functions estimate_probabilities() and calc_perplex(). The file is created locally.

#
#detect_lang_for_text("the deepening of the internal market")
#detect_lang_for_text("la comisin debe continuar avanzando por este")
#detect_lang_for_text("bestehende system der europischen wettbewerbsregel")

# Q5 part1
test_doc="test"
detect_lang_test_doc(test_doc)
#detect_lang_for_text("this is not my day")
#generate_random()
...
#file.write("xyzxxyyzzzxxxxyxyxxzxxzyzyzzzxxxxxyxyxyxzyzyxyxxyzyzzyxyxxyxyzyzyxxyxyxzyyxz")
#file.close()
#
#unigram_count = count_unigrams(file_name)
##print "\n Debug : The unigram count dictionary is \n ", unigram_count , "\n"
#
#bigram_count = count_bigrams(file_name)
##print "\n Debug : The bigram count dictionary is \n ", bigram_count , "\n"
#
#trigram_count = count_trigrams(file_name)
##print "\n Debug : The trigram count dictionary is \n ", trigram_count , "\n"

#get_history_english("th")
#get_history_english("an")

#prob_dict=estimate_probabilities(bigram_count,trigram_count)
##print_estimated_prob(prob_dict)
#
#add1_prob_dict = estimate_probabilities_with_add1(bigram_count,trigram_count)
##print_estimated_prob(add1_prob_dict)
#
#log_prob_dict = estimate_log_probabilities_with_add1(bigram_count,trigram_count)
##print_estimated_prob(log_prob_dict)
#
##print "Debug : Perplexity without add1 and without log is ",calc_perplex(text,prob_dict)
##print "Debug : Perplexity with add1 and without log is ",calc_perplex(text,add1_prob_dict)
##print "Debug : Perplexity with add1 and with log is ",calc_perplex_log(text,log_prob_dict)
...
##print prob_dict
#text1='[[abaab]'
#print "Comment : Perplexity for text = ",text1, "is",calc_perplex(text1,prob_dict)
#text2= '[ba]'
#print "Comment : Perplexity for text = ",text2, "is",calc_perplex(text2,prob_dict)
...
##print prob_dict
#text3='[[abaab]'
#print "Comment : Perplexity  using log probabilities for text = ",text3, "is",calc_perplex_log(text3,log_prob_dict)
#text4= '[ba]'
#print "Comment : Perplexity  using log probabilities for text = ",text4, "is",calc_perplex_log(text4,log_prob_dict)
...
#norm_prob_dict = normalize_probabilities(prob_dict)
#print_estimated_prob(norm_prob_dict)
...
#        print ("\n Error : The probability dictionary is empty. Program aborted. \n")
#        sys.exit()
#    sum=get_sum_prob(prob_dict)
#    for outer_key in vocab:
#        inner_dict = prob_dict[outer_key]
#        for inner_key1 in vocab:
#            for inner_key2 in vocab:
#                inner_key=inner_key1+inner_key2
#                current_prob = inner_dict[inner_key]
#                inner_dict[inner_key]=current_prob/sum
#    return prob_dict
